...
class LaneATT(nn.Module):
    ...
    def forward(self, x, conf_threshold=None, nms_thres=0, nms_topk=3000):
        ...
        # Apply nms
        #proposals_list, scores = self.nms(reg_proposals, attention_matrix, nms_thres, nms_topk, conf_threshold)
        proposals_list = self.nms(reg_proposals, attention_matrix, nms_thres, nms_topk, conf_threshold)


        return proposals_list#, scores
    ...
    def proposals_to_pred(self, proposals):
        ...
        for lane in proposals:
            ...
            #print(j) 
            lanes.append(lane)
            #print('lanes data is',lanes)
              	
        return lanes

    #def findCenterLine(lanes):
    	#if lanes>=2:
    ...
    def decode(self, proposals_list, as_lanes=False):
        ...
        for proposals, _, _, _ in proposals_list:
            ...
            if as_lanes:
                ...
                #    print(pred[i])
                if len(pred)>=2:
                    egoLanes.append(pred[0])
                    egoLanes.append(pred[1])
                    #print(egoLanes[0].points[:][0],'\n')   #type(egoLanes[0]) : <class 'lib.lane.Lane'>    #type(egoLanes[0].points) : <class 'numpy.ndarray'>
                    x1=[]
                    y1=[]
                    x2=[]
                    y2=[]
                    ...
                    for n in range(len(egoLanes[0].points)):
                        x1.append(egoLanes[0].points[n][0])
                        y1.append(egoLanes[0].points[n][1])
                    x1=np.array(x1)
                    y1=np.array(y1)
                    ...
                    #root1=np.roots(z1s)
                    #z1f=P.polyfromroots(root1)
                    #p1f=np.poly1d(z1f)
                    #print(z1, z1f)
                    for k in range(len(egoLanes[1].points)):
                        #print('len is', len(egoLanes[1].points))
                        x2.append(egoLanes[1].points[k][0])
                        y2.append(egoLanes[1].points[k][1])
                    x2=np.array(x2)
                    y2=np.array(y2)
                    ...
                    st=max(egoLanes[0].points[0][1], egoLanes[1].points[0][1])
                    #print('st is',st)
                    ed=min(egoLanes[0].points[-1][1], egoLanes[1].points[-1][1])
                    #print('ed is', ed)
                    desiredInterval=(ed-st)/10
                    ...
                    #for p in range(10):
                            ...
                            #xNew1=p1(st)     #now we have(x,ynew)
                            #xNew2=p2(st)
                            #xNew1=np.roots(p1-st) 
                            #xNew1=xNew1[xNew1>=0]    #now we have(x,ynew)
                            #xNew1=xNew1[xNew1 <= 1]
                            #xNew1=xNew1[np.isreal(xNew1)].real

                            #xNew2=np.roots(p2-st) 
                            #xNew2=xNew2[xNew2>=0]
                            #xNew2=xNew2[xNew2 <=1]
                            #xNew2=xNew2[np.isreal(xNew2)].real
                            #print('xNews',xNew1,xNew2, type(xNew1))
                            #xMid.append((xNew1+xNew2)/2)
                            #print("1")
                            #st+=0.05
                    #print(type(xMid), "type of xmid")
                    #xMid=np.array(xMid, dtype=np.float32)
                    #yNew=np.array(yNew)
                    #print("2")
                    #print(type(xMid))

                    #xMid=torch.from_numpy(xMid)
                    #print('xMid is', xMid)
                    #yNew=torch.from_numpy(yNew)
                    #if len((xMid.reshape(-1, 1))[0])==len((yNew.reshape(-1, 1))[0]):
                    #if (xMid.reshape(-1, 1).shape)[0]== (yNew.reshape(-1, 1).shape)[0]:
                        #print((xMid.reshape(-1, 1)).shape[0])
                        #print((yNew.reshape(-1, 1)).shape)[0])
                        #ptsMid = torch.stack((xMid.reshape(-1, 1), yNew.reshape(-1, 1)), dim=1).squeeze(2)
                        #ptsMid=ptsMid.detach().cpu().numpy()
                        #ptsMid=[]

                    #print('print ptsmid',ptsMid)
                    #print(desiredInterval, '\n')
                    #midlane = Lane(points=ptsMid.numpy())
